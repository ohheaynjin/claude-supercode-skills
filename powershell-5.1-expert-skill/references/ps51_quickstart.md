...
**해결책:** DSC 모듈 가져오기:```powershell
Import-Module PSDesiredStateConfiguration
```

## 모범 사례

1. 항상 스크립트 상단에 `#Requires -Version 5.1`을(를) 사용하세요
2. 오류 처리를 위한 try/catch/finally 블록 구현
3. 디버깅 정보를 보려면 `Write-Verbose`을 사용하세요.
4. 검증 속성으로 입력 매개변수 검증
5. 파괴적인 작업에는 `ShouldProcess`을 사용하세요.
6. 먼저 비프로덕션 환경에서 스크립트를 테스트하세요.

## 다음 단계

- 고급 패턴을 보려면 `references/` 디렉터리를 탐색하세요.
- PowerShell 5.1 관련 기술에 대한 `legacy_patterns.md` 검토
- PowerShell 7로 업그레이드하려면 `migration_guide.md`을 확인하세요.
