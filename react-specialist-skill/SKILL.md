...
| **재렌더링 폭풍** | 구성 요소가 초당 10회 이상 렌더링됨 | React.memo, useMemo | 80%+ 감소 |
| **대형 번들** | JS 번들 >500KB | 트리 흔들기, 동적 가져오기 | 30-50% 더 작아짐 |
| **느린 목록 렌더링** | 목록 >1000개 항목 지연 | 반응 창/반응 가상화 | 90% 이상 더 빨라짐 |
| **비용이 많이 드는 계산** | 상호 작용 시 CPU 스파이크 | useMemo, 웹 워커 | 50-70% 더 빨라짐 |
...
| 사용 사례 | 패턴 | 복잡성 | 유연성 | 예 |
|------------|---------|------------|-------------|---------|
| **간단한 UI** | 소품 + 어린이 | 낮음 | 낮음 |`<Button>Click</Button>`|
| **구성** | 소품 개체 | 낮음 | 중간 |`<Button config={{...}} />`|
| **복잡한 구성** | 복합 구성 요소 | 중간 | 높음 |`<Tabs><Tab /></Tabs>`|
| **렌더링 유연성** | 렌더링 소품 | 중간 | 매우 높음 |`<List render={...} />`|
| **헤드리스 UI** | 맞춤형 후크 | 높음 | 최대 |`useSelect()`|
| **다형성** |`as`소품 | 중간 | 높음 |`<Text as="h1" />`|
