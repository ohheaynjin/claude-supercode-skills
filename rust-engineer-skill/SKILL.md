...
| **HTTP 프레임워크** | 악숨, 하이퍼, 토닉 | 조류 | 공식 없음 |
| **양자** | 프로덕션(Discord, AWS) | 실험적 | 벽감 |
| **최적의 용도** | 생산 서비스 | 프로토타이핑 | 임베디드 |
...
| 상황 | 결정 | 이론적 해석 |
|-----------|----------|-----------|
| **레거시 C 라이브러리** | FFI 래퍼 | 테스트된 코드를 다시 구현하지 마세요 |
| **성능이 중요한 C** | 벤치마크 우선 | Rust는 C와 일치/초과할 수 있습니다. |
| **간단한 C 알고리즘** | Rust로 다시 작성하기 | 유지 관리가 더 쉬워졌습니다. |
| **OS별 API** | FFI via `windows-rs` | 순수 Rust 대안 없음 |
| **C/Python에서 Rust 호출하기** | FFI with `#[no_mangle]` | 교차 언어 사용 활성화 |

## 에스컬레이션 트리거

**위험 신호 → `oracle`(으)로 에스컬레이션하세요.**
...
- **도구**: Docker 다단계 빌드, Prometheus 측정항목(`axum-prometheus`을 통해)
...
- **도구**: FFI 바인딩의 경우 `bindgen`, 양방향 C++/Rust 상호 운용성의 경우 `cxx`
